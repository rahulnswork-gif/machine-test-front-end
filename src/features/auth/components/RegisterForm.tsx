          ...
          <CardContent>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
              ...
              <div className="space-y-2">
                <Label htmlFor="name" className="text-slate-700 font-medium">{t('auth.name')}</Label>
                <Input 
                  id="name" 
                ...
                {form.formState.errors.name && (
                  <p className="text-xs text-git-danger font-medium">{form.formState.errors.name.message}</p>
                )}
              </div>
              <div className="space-y-2">
              ...
              <Button 
                ...
                disabled={registerMutation.isPending}
              >
                {registerMutation.isPending ? 'Creating account...' : t('auth.submit')}
              </Button>
            </form>
          </CardContent>
          <CardFooter className="flex justify-center pt-6 border-t border-slate-100">
            <p className="text-sm text-slate-600">
              {t('auth.haveAccount')} <Link to="/login" className="text-git-primary hover:text-git-primary/80 font-semibold hover:underline">{t('auth.login')}</Link>
            </p>
          </CardFooter>
        </Card>
      </div>
